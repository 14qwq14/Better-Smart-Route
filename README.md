...
如果你安装了 [ModConfig](https://steamcommunity.com/sharedfiles/filedetails/?id=3249021469)，可以在游戏的“模组设置”菜单中直接调整此模组的配置：

- 启用/禁用特定路线（例如可以开启默认被隐藏的“首领速通”和“最大收益”路线）
- 细调路线的高亮颜色和优先级
- 设置各房间类型的目标计分权重

如果你没有安装 ModConfig，你可以在用户目录/游戏目录下的 `mods/RouteSuggestConfig.json` 里手动修改配置。
...
| `target_counts`  | Object  | 各节点种类的目标数量（目前使用平方惩罚误差，使搜索尽量贴近目标个数）。 |

**支持的房间类型（用于 `target_counts`）：**
